...
namespace AllProgrammic\Component\Resque\Charts\Process;
...
class Redis implements ChartsInterface
{
    ...
    public function clean($interval = 7)
    {
        ...
        foreach ($data as $value) {
            if ($value['date'] < $date) {
                $this->backend->del(sprintf('%s:%s', Charts::INDEX_PROCESS, $value['date']));
            }
        }
    }
    ...
    {
        return $this->backend->incrBy(sprintf('%s:%s', Charts::INDEX_PROCESS, strtotime(date('Y-m-d'))), 1);
    }
    ...
    public function peek($start = 0, $count = 1)
    {
        ...
        foreach ($this->backend->scanLoop(sprintf('%s:*', Charts::INDEX_PROCESS)) as $result) {
            $time = substr($result, strlen(sprintf('%s%s:', $this->backend->getNamespace(), Charts::INDEX_PROCESS)));

            $data[] = [
                'date'  => (int) $time,
                'count' => (int) $this->backend->get(sprintf('%s:%s', Charts::INDEX_PROCESS, $time)),
            ];
        }
